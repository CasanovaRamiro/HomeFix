...
describe('post.service - completePost', () => {
    ...
    it('completa el post cuando estÃ¡ In progress', async () => {
        vi.mocked(findPostById).mockResolvedValue(mockPost);
        vi.mocked(updatePostStatus).mockResolvedValue({ id: 'uuid-1', status: 'Completed' });
    ...
    it('lanza 400 si el post no está In progress o Active', async () => {
        vi.mocked(findPostById).mockResolvedValue({ ...mockPost, status: 'Paused' });
...
describe('post.service - reopenPost', () => {
    ...
    it('reabre el post y resetea la aplicaciÃ³n aceptada a Pending', async () => {
        vi.mocked(findPostById).mockResolvedValue(mockInProgress);
        vi.mocked(findAcceptedApplications).mockResolvedValue([{
                id: 'app-1',
                status: 'Accepted',
                createdAt: new Date(),
        ...
        vi.mocked(updatePostStatus).mockResolvedValue({ id: 'uuid-1', status: 'Active' });
    ...
    it('lanza 400 si el post no estÃ¡ In progress', async () => {
        vi.mocked(findPostById).mockResolvedValue({ ...mockInProgress, status: 'Active' });
...
describe('post.service - createSubContract', () => {
    ...
    const createMockSubPost = (overrides = {}) => ({
    ...
    it('creates one subcontract per position without parentPostId', async () => {
        ...
        vi.mocked(createSubPost).mockResolvedValue(createMockSubPost({ parentPostId: undefined, title: 'SubcontrataciÃ³n - Pintura' }));
...
describe('post.service - markInProgress', () => {
    ...
    it('should reject if no hired workers', async () => {
        vi.mocked(findPostById).mockResolvedValue(mockPost);
        vi.mocked(findPostCategories).mockResolvedValue([{ id: 'cat-1', postId, quantity: 2, filledCount: 0 }]);
    ...
    it('should update status to InProgress for client post', async () => {
        vi.mocked(findPostById).mockResolvedValue(mockPost);
        vi.mocked(findPostCategories).mockResolvedValue([{ id: 'cat-1', postId, quantity: 2, filledCount: 1 }]);
    ...
    it('should cascade to all posts in subcontractGroupId', async () => {
        ...
        vi.mocked(findPostCategories).mockResolvedValue([{ id: 'cat-1', postId, quantity: 1, filledCount: 1 }]);
...
describe('post.service - createPost emergency', () => {
    ...
    it('creates emergency post and broadcasts to workers', async () => {
        ...
        expect(createPost).toHaveBeenCalledWith(expect.objectContaining({ emergencyExpiresAt: expect.any(Date) }));
        expect(broadcastEmergency).toHaveBeenCalledWith(expect.anything(), createdEmergencyPost.id, emergencyInput.title, emergencyInput.description, emergencyInput.categoryId);
...
describe('post.service - createEmergencyPost', () => {
    ...
    it('throws validation error when title is empty', async () => {
        await expect(postService.createEmergencyPost({ ...emergencyInput, title: '' })).rejects.toThrow('title is required');
    });
});
describe('post.service - validatePostInput', () => {
    it('skips date validation when isEmergency is true', () => {
        expect(() => postService.validatePostInput({
            userId: 'u-1',
            title: 'Emergency',
            description: 'Urgent',
            address: 'Calle 1',
...
describe('post.service - cancelPost with accepted application', () => {
    ...
    it('notifies accepted worker when cancelling', async () => {
        vi.mocked(findPostById).mockResolvedValue(activePost);
        vi.mocked(updatePostStatus).mockResolvedValue({ ...activePost, status: 'Cancelled' });
        vi.mocked(findAcceptedApplications).mockResolvedValue([{
                id: 'app-1', workerId: 'worker-1', postId: 'uuid-1', status: 'Accepted',
            }]);
        await postService.cancelPost('uuid-1', 'user-uuid-1');
        expect(notifyUser).toHaveBeenCalledWith(expect.anything(), 'worker-1', 'post_cancelled', { postTitle: 'ReparaciÃ³n de caÃ±o' });
    });
});
describe('post.service - finalizePost with accepted application', () => {
    ...
    it('marks accepted application as Completed before finalizing', async () => {
        ...
        vi.mocked(updateApplicationStatus).mockResolvedValue({ id: 'app-1', status: 'Completed' });
        vi.mocked(updatePostStatus).mockResolvedValue({ ...pausedPost, status: 'Completed' });
        await postService.finalizePost('uuid-1', 'user-uuid-1');
        expect(updateApplicationStatus).toHaveBeenCalledWith('app-1', 'Completed');
        expect(notifyUser).toHaveBeenCalledWith(expect.anything(), 'worker-1', 'post_completed', { postTitle: 'ReparaciÃ³n de caÃ±o' });
    });
});
describe('post.service - completePost with accepted application', () => {
    ...
    it('marks accepted application as Completed before completing', async () => {
        vi.mocked(findPostById).mockResolvedValue(inProgressPost);
        vi.mocked(findAcceptedApplications).mockResolvedValue([{
                id: 'app-1', workerId: 'worker-1', postId: 'uuid-1', status: 'Accepted',
            }]);
        vi.mocked(updateApplicationStatus).mockResolvedValue({ id: 'app-1', status: 'Completed' });
        vi.mocked(updatePostStatus).mockResolvedValue({ ...inProgressPost, status: 'Completed' });
        await postService.completePost('uuid-1', 'user-uuid-1');
        expect(updateApplicationStatus).toHaveBeenCalledWith('app-1', 'Completed');
        expect(notifyUser).toHaveBeenCalledWith(expect.anything(), 'worker-1', 'post_completed', { postTitle: 'ReparaciÃ³n de caÃ±o' });
    });
