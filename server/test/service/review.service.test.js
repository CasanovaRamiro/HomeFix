...
describe('createWorkerReview', () => {
    ...
    it('should create a review without optional description', async () => {
        const inputWithoutDesc = { postId, rating: 4 };
        vi.mocked(findPostById).mockResolvedValue(mockPost);
        vi.mocked(findAcceptedApplications).mockResolvedValue([mockAcceptedApp]);
...
describe('createWorkerReview (dismissed worker by applicationId)', () => {
    ...
    const dismissInput = { postId, applicationId, rating: 1, description: 'No se presentÃ³' };
    it('creates a review for a dismissed worker', async () => {
        vi.mocked(findApplicationById).mockResolvedValue(dismissedApplication);
    ...
    it('throws 400 when the application is not Dismissed', async () => {
        vi.mocked(findApplicationById).mockResolvedValue({ ...dismissedApplication, status: 'Accepted' });
...
describe('createClientReview', () => {
    ...
    it('should create a client review successfully', async () => {
        vi.mocked(findApplicationById).mockResolvedValue(mockApplication);
    ...
    it('should throw 404 when application is not found', async () => {
        vi.mocked(findApplicationById).mockResolvedValue(null);
        await expect(createClientReview(applicationId2, workerId2, { applicationId: applicationId2, rating: 5 })).rejects.toMatchObject({ status: 404, message: 'Application not found' });
        expect(createClientReviewData).not.toHaveBeenCalled();
    });
    it('should throw 403 when user is not the application worker', async () => {
        vi.mocked(findApplicationById).mockResolvedValue(mockApplication);
        await expect(createClientReview(applicationId2, 'other-worker-id', { applicationId: applicationId2, rating: 5 })).rejects.toMatchObject({ status: 403, message: 'Forbidden' });
        expect(createClientReviewData).not.toHaveBeenCalled();
    });
    it('should throw 400 when post is not completed', async () => {
        ...
        await expect(createClientReview(applicationId2, workerId2, { applicationId: applicationId2, rating: 5 })).rejects.toMatchObject({
            status: 400,
