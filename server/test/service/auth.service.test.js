...
import { loginUser, registerUser, registerWorker, syncAuth0User, forgotPassword, resendVerificationEmail } from '../../src/domain/services/auth.service.js';
const mockUser = {
    id: 'uuid-jane',
    name: 'Jane',
    email: 'jane@test.com',
...
describe('auth.service - syncAuth0User', () => {
    ...
    it('throws 404 when email does not exist and isRegistration is false', async () => {
        vi.mocked(userData.findByEmail).mockResolvedValue(null);
        await expect(syncAuth0User({ sub: 'auth0|abc123', email: 'jane@test.com', name: 'Jane' })).rejects.toMatchObject({ status: 404 });
    });
    it('creates user with role client when isRegistration is true', async () => {
        ...
        expect(userData.createUser).toHaveBeenCalledWith(expect.objectContaining({ role: UserRole.Client }));
    });
    it('throws 401 when claims has no sub', async () => {
        await expect(syncAuth0User({ email: 'jane@test.com' })).rejects.toMatchObject({ status: 401 });
    });
    ...
    it('migrates ghost user when real email arrives and ghost exists', async () => {
        ...
        vi.mocked(userData.createUser).mockResolvedValue(mockUser);
        await syncAuth0User({ sub: 'auth0|abc123', email: 'jane@test.com', name: 'Jane' }, true);
        expect(userData.updateUserByEmail).toHaveBeenCalledWith('auth0|abc123@auth0.local', expect.objectContaining({ email: 'jane@test.com' }));
    });
    it('updates role when existing user has role "user"', async () => {
        ...
        expect(userData.updateUserByEmail).toHaveBeenCalledWith('jane@test.com', expect.objectContaining({ role: 'worker' }));
    });
    it('updates name when existing user name equals the sub claim', async () => {
        ...
        expect(userData.updateUserByEmail).toHaveBeenCalledWith('jane@test.com', expect.objectContaining({ name: 'Jane' }));
    });
});
describe('auth.service - registerUser', () => {
    ...
    it('throws if required fields are missing', async () => {
        await expect(registerUser({})).rejects.toThrow('El nombre es obligatorio');
    });
    it('throws if email is missing', async () => {
        await expect(registerUser({ name: 'Jane' })).rejects.toThrow('El correo electrónico es obligatorio');
    });
    it('throws if password is missing', async () => {
        await expect(registerUser({ name: 'Jane', email: 'jane@test.com' })).rejects.toThrow('La contraseña es obligatoria');
    });
});
describe('auth.service - registerWorker', () => {
    beforeEach(() => {
        ...
        vi.mocked(userData.addUserCategories).mockResolvedValue(undefined);
    });
    it('registers a worker with categories', async () => {
        ...
        expect(result.email).toBe('jane@test.com');
        expect(userData.createUser).toHaveBeenCalledWith(expect.objectContaining({ role: UserRole.Worker }));
    ...
    it('throws 400 when no categories are provided', async () => {
        await expect(registerWorker({ name: 'Bob', email: 'bob@test.com', password: 'Password123!', categories: [] })).rejects.toMatchObject({ status: 400 });
    });
    it('throws 409 when email already exists', async () => {
        vi.mocked(userData.findByEmail).mockResolvedValue(mockUser);
        await expect(registerWorker({ name: 'Bob', email: 'bob@test.com', password: 'Password123!', categories: ['Plumbing'] })).rejects.toMatchObject({ status: 409 });
    });
...
describe('auth.service - forgotPassword', () => {
    ...
    it('propagates error when Auth0 call fails', async () => {
        vi.mocked(auth0Provider.sendAuth0PasswordReset).mockRejectedValue(Object.assign(new Error('Auth0 error'), { status: 502 }));
...
describe('auth.service - loginUser', () => {
    ...
    it('throws 401 when credentials are invalid', async () => {
        vi.mocked(auth0Provider.loginWithAuth0).mockRejectedValue(Object.assign(new Error('invalid_grant'), { status: 401 }));
