...
describe("AI Suggestion Service", () => {
    ...
    describe("basic functionality", () => {
        it("should generate a response", async () => {
            mockGenerateContent.mockResolvedValue({
                response: {
                    text: () => '{"type":"question","text":"¿Desde cuándo ocurre el problema?"}',
                },
            });
        ...
        it("should return either question or suggestion type", async () => {
            mockGenerateContent.mockResolvedValue({
                response: {
                    text: () => '{"type":"suggestion","data":{"suggestedTitle":"Problema eléctrico","suggestedCategoryId":2,"suggestedCategoryName":"Electricidad","possibleIssue":"Probablemente una fuga eléctrica.","confidence":"high"}}',
                },
            });
            const response = await suggestPost({
        ...
        it("should process user messages correctly", async () => {
            mockGenerateContent.mockResolvedValue({
                response: {
                    text: () => '{"type":"question","text":"¿Ocurre con varios dispositivos?"}',
                },
            });
            const response = await suggestPost({
                messages: [{ role: "user", text: "Me da corriente" }],
    ...
    describe("suggestion flow", () => {
        it("should generate a category suggestion", async () => {
            mockGenerateContent.mockResolvedValue({
                response: {
                    text: () => '{"type":"suggestion","data":{"suggestedTitle":"Cortocircuito","suggestedCategoryId":2,"suggestedCategoryName":"Electricidad","possibleIssue":"Posible falla eléctrica.","confidence":"high"}}',
                },
            });
            const response = await suggestPost({
        ...
        it("should generate a suggested title", async () => {
            mockGenerateContent.mockResolvedValue({
                response: {
                    text: () => '{"type":"suggestion","data":{"suggestedTitle":"Pérdida de agua","suggestedCategoryId":1,"suggestedCategoryName":"Plomeria","possibleIssue":"Probable fuga.","confidence":"medium"}}',
                },
            });
            const response = await suggestPost({
        ...
        it("should generate a possible issue description", async () => {
            mockGenerateContent.mockResolvedValue({
                response: {
                    text: () => '{"type":"suggestion","data":{"suggestedTitle":"Fuga","suggestedCategoryId":1,"suggestedCategoryName":"Plomeria","possibleIssue":"Posiblemente haya una fuga interna.","confidence":"medium"}}',
                },
            });
            const response = await suggestPost({
        ...
        it("should return a confidence level", async () => {
            mockGenerateContent.mockResolvedValue({
                response: {
                    text: () => '{"type":"suggestion","data":{"suggestedTitle":"Problema eléctrico","suggestedCategoryId":2,"suggestedCategoryName":"Electricidad","possibleIssue":"Posible fuga.","confidence":"high"}}',
                },
            });
            const response = await suggestPost({
                messages: [{ role: "user", text: "Me da corriente" }],
        ...
        it("should include normalized empty fields", async () => {
            mockGenerateContent.mockResolvedValue({
                response: {
                    text: () => '{"type":"suggestion","data":{"suggestedTitle":"Problema","suggestedCategoryId":2,"suggestedCategoryName":"Electricidad","possibleIssue":"Posible problema.","confidence":"high"}}',
                },
            });
            const response = await suggestPost({
                messages: [{ role: "user", text: "Me da corriente" }],
    ...
    describe("conversation handling", () => {
        it("should throw if no messages are provided", async () => {
            await expect(suggestPost({ messages: [] })).rejects.toThrow("Se requiere al menos un mensaje para generar una sugerencia");
        });
