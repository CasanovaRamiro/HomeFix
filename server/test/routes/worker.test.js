...
beforeEach(() => cleanDb());
const makeWorker = (email, name) => createUser(email, name, 'hashed', { role: UserRole.Worker });
const makeUser = (email, name) => createUser(email, name, 'hashed', { role: 'user' });
describe('GET /workers', () => {
    it('returns all workers when authenticated', async () => {
        ...
        res.body.forEach((w) => expect(w.role).toBe(UserRole.Worker));
    });
