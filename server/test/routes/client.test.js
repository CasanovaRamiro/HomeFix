...
const createPost = (userId, status, overrides = {}) => prisma.post.create({
    data: {
        userId,
        title: 'Test Post',
...
const createApplication = (postId, status) => prisma.application.create({
    data: { workerId, postId, status },
});
const createWorkerReview = (applicationId, rating) => prisma.workerReview.create({
    data: {
        applicationId,
        reviewerId: clientId,
        workerId,
...
const createClientReview = (applicationId, rating) => prisma.clientReview.create({
    data: {
        applicationId,
        reviewerId: workerId,
        clientId,
...
describe('GET /client/posts', () => {
    ...
    it('retorna solo posts Completed y Cancelled (excluye Active)', async () => {
        ...
        expect(res.body.data.map((p) => p.status).sort())
