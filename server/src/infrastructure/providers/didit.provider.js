...
export const createDiditSession = async (vendorData) => {
    const apiKey = env.DIDIT_API_KEY;
    const workflowId = env.DIDIT_WORKFLOW_ID;
    if (!apiKey)
        throw createHttpError(500, 'DIDIT_API_KEY is not configured');
    if (!workflowId)
        throw createHttpError(500, 'DIDIT_WORKFLOW_ID is not configured');
...
export const getDecision = async (sessionId) => {
    ...
    return {
        ...
        sessionKind: data.session_kind ?? 'user',
        vendorData: data.vendor_data ?? null,
        idVerifications: data.id_verifications ?? [],
        livenessChecks: data.liveness_checks ?? [],
        faceMatches: data.face_matches ?? [],
        amlScreenings: data.aml_screenings ?? [],
    };
};
