...
import { createWorkerReview, createClientReview, findWorkerReviewByApplication } from '../../domain/services/review.service.js'
...
router.get('/application/:applicationId', async (req, res, next) => {
  try {
    ...
    const review = await findWorkerReviewByApplication(req.params.applicationId, user.id)
    res.json(review)
  } catch (error) {
