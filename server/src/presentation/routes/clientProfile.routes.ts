...
import type { UpdateClientProfileRequest } from '../types/clientProfile.types.js'
...
router.patch('/:id', async (req, res, next) => {
  try {
    ...
    const body = req.body as UpdateClientProfileRequest
    const input: UpdateClientProfileInput = {}
    if (body.name !== undefined) input.name = body.name
    if (body.surname !== undefined) input.surname = body.surname
    if (body.phone !== undefined) input.phone = body.phone
    if (body.bio !== undefined) input.bio = body.bio
    if (body.photo !== undefined) input.photo = body.photo
