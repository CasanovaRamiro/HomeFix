        ...
        if (header?.startsWith('Bearer ')) {
            ...
            if (sub) {
                ...
                if (cached && Date.now() < cached.expiresAt) {
                    if (req.auth?.payload) {
                        if (cached.email)
                            req.auth.payload['email'] = cached.email;
                        if (cached.name)
                            req.auth.payload['name'] = cached.name;
                        if (cached.nickname)
                            req.auth.payload['nickname'] = cached.nickname;
                    }
                }
                else {
                    try {
                        ...
                        if (resp.ok) {
                            ...
                            if (req.auth?.payload) {
                                ...
                                if (userinfo.name)
                                    req.auth.payload['name'] = userinfo.name;
                                if (userinfo.nickname)
                                    req.auth.payload['nickname'] = userinfo.nickname;
                            }
                        }
                    }
                    catch {
